...
.product-container {
    ...
    justify-content: space-evenly; /* Canh chỉnh các khung để chúng lấp đầy không gian */
}
...
.button {
    display: inline-block; /* Thiết lập nền màu chỉ có độ rộng tối thiểu cần thiết */
    padding: 8px 20px; /* Thiết lập padding cho phần nền của nút */
    font-size: 15px; 
...
button#category:hover {
    ...
    <div class="filter-container" style="margin-left: 10px;">
        ...
        <select id="categoryDropdown">
            <option value="0">Chọn nội dung bạn cần</option>
            <?php
            // Truy vấn danh sách thể loại từ cơ sở dữ liệu
            $sql_categories = "SELECT * FROM theloai";
            $result_categories = $connect->query($sql_categories);
            // Hiển thị các tùy chọn dropdown cho các thể loại
            while ($row_category = $result_categories->fetch_assoc()) {
                echo "<option value='" . $row_category['ID_TheLoai'] . "'>" . $row_category['TenTheLoai'] . "</option>";
            }
            ?>
        </select>
        <button onclick="filterPosts()" id="category">Lọc</button>
    </div>
    <hr>
	<div class="gtco-section" style='padding: 2em 2em'>
   <?php
        // Truy vấn dữ liệu từ bảng baiviet với điều kiện thể loại (nếu có)
        if (isset($_GET['category']) && $_GET['category'] != "0") {
            $category_condition = "AND ID_TheLoai = " . $_GET['category'];
        } else {
            $category_condition = "";
        }

        // Truy vấn dữ liệu từ bảng baiviet với điều kiện thêm các thể loại được chọn (nếu có)
        if (isset($_GET['categories']) && !empty($_GET['categories'])) {
            $selected_categories = explode(",", $_GET['categories']);
            $category_condition .= " AND ID_TheLoai IN (" . implode(",", $selected_categories) . ")";
        }
        ...
        // Tính toán OFFSET (vị trí bắt đầu của bài viết trên trang hiện tại)
        $offset = ($current_page - 1) * $posts_per_page;

        // Truy vấn dữ liệu từ bảng baiviet với LIMIT và OFFSET và điều kiện thể loại
        $sql = "SELECT * FROM baiviet WHERE AnHien = 1 $category_condition LIMIT $posts_per_page OFFSET $offset";
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                echo "<h3 style='text-align: center;'>" . $row['TieuDe'] . "</h3>";
                ...
                echo "<a href='newsdetail.php?id=" . $row['ID_News'] . "' target='_blank' class='button'>Xem chi tiết</a>";
            ...
            // Phân trang
            $sql_total = "SELECT COUNT(*) AS total FROM baiviet WHERE AnHien = 1 $category_condition";
	...
	<script src="js/main.js"></script>
	    <script>
        // Xử lý sự kiện thay đổi dropdown và checkbox
        function filterPosts() {
            ...
            // Xây dựng URL mới với các thể loại đã chọn
            var newUrl = "News.php";
