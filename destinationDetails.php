...
.comment-container form input[type="submit"] {
    ...
    cursor: pointer;
    border-radius: 3px;
	transition: background-color #FFA500 1s;
}
...
if (isset($_GET['id'])) {
    ...
    $sql_tour_details = "SELECT tours.*, 
                            huongdanvien.ID_HuongDanVien, 
                            huongdanvien.TenHuongDanVien, 
                            huongdanvien.SoDienThoai, 
                            huongdanvien.Email, 
                            huongdanvien.NoiSinh, 
                            huongdanvien.DiaChi, 
                            huongdanvien.NgaySinh, 
                            huongdanvien.CCCD, 
    ...
    if ($result_tour_details->num_rows > 0) {
        ...
        while ($row = $result_tour_details->fetch_assoc()) {
			echo '<div class="tour-header">';
				echo '<div class="tour-info-header">';
					echo '<h1 style="margin: 10px;">' . $row['TenTour'] . '</h1>';
					echo '<h2 style="color: #09C6AB; margin-right: 20px; margin: 10px"><strong><i class="fas fa-money-bill" style="margin-right: 10px;"></i>Giá:</strong> ' . number_format($row['Gia'], 0, ',', '.') . ' VND/ Người</h2>';					
				echo '</div>';
			echo '<hr>';
			echo '</div>';
			...
			echo '<div class="image-frame" style="border-radius: 40px;">';
			echo '<img src="/BayWithVaa/img/' . $row['HinhAnh'] . '" alt="' . $row['TenTour'] . '" class="tour-image" style="border-radius: 10px;">';
			// Hiển thị danh sách ảnh chi tiết
			echo '<h6 style="margin-top: 10px;">Click vào ảnh bên dưới để xem thêm</h6>';
			...
			if ($result_tour_images->num_rows > 0) {
				while ($row_image = $result_tour_images->fetch_assoc()) {
					echo '<img src="/BayWithVaa/img/' . $row_image['Images'] . '" alt="Additional Image" class="additional-image" style="border-radius: 10px;">';
				}
			}
    		echo '</div>';
			echo '</div>';
			...
			echo '<div class="additional-content">'; // Mở div mới chứa thông tin về hướng dẫn viên
			...
			echo '<hr>';
			echo '<h6 style="text-align: center;">Nhấp vào bên dưới để xem chi tiết</h6>';
			echo '<div class="description-frame">'; // Khung trắng chứa mô tả
			echo '<h3 style="color: #09C6AB; cursor: pointer;" onclick="toggleNoiDung()">Địa điểm</h3>';
			echo '<div id="noidung" class="tour-description" style="display: none;">' . $row['NoiDung'] . '</div>';
			echo '</div>';
			
			echo '<div class="description-frame">'; // Khung trắng chứa mô tả
			echo '<h3 style="color: #09C6AB; cursor: pointer;" onclick="toggleMoTa()">Lịch Trình</h3>';
...
if ($result_other_tours->num_rows > 0) {
	...
	echo '<h2 style="text-align: center; font-weight: bold;"><i>Có thể bạn sẽ thích</i></h2>';
	...
	while ($row_other = $result_other_tours->fetch_assoc()) {
		...
		echo '<h3 style="text-align: center;">' . $row_other['TenTour'] . '</h3>';
		echo '<div style="max-width:399px; min-heigth:260px">';
		// Hiển thị hình ảnh của tour
			echo '<img style="max-width:399px; min-heigth:260px" src="/BayWithVaa/img/' . $row_other['HinhAnh'] . '" alt="' . $row_other['TenTour'] . '">';
		echo '</div>';
		...
		if ($other_average_rating) {
			echo '<i class="other-tour-rating" style="font-weight: bold;">Đánh giá: ' . number_format($other_average_rating, 1) . ' <i class="fas fa-star"></i></i>';
		} else {
			echo '<i class="other-tour-rating" style="font-weight: bold;">Chưa có đánh giá</i>';
		}

		// Hiển thị giá cả của tour
		echo '<p style="color: #09C6AB;"><strong>Giá:</strong> ' . number_format($row_other['Gia'], 0, ',', '.') . ' VND</p>';
		// Thêm liên kết đến trang destinationDetails.php với ID của tour
		echo '<a href="destinationDetails.php?id=' . $row_other['ID_Tour'] . '" target="_blank" style="font-weight: bold; border-radius: 20px;">Xem chi tiết</a>';
		echo '</div>';
	}
...
if (isset($_SESSION['email'])) {
  ...
  echo '<select name="rating" id="rating" style="border-radius: 10px;">';
    ...
    echo '</select>';
    echo '<textarea name="comment" id="comment" rows="4" placeholder="Viết bình luận của bạn..." style="border-radius: 10px;"></textarea>';
    echo '<input type="submit" value="Gửi" style="border-radius: 20px; width: 7%; font-weight: bold;">';
    echo '</form>';
} else {
    ...
    <div class="comment-list">
        ...
        if ($result_comments->num_rows > 0) {
            while ($comment = $result_comments->fetch_assoc()) {
				...
				echo '<i>' . date('d-m-Y / H:i:s', strtotime($comment['NgayPhanHoi'])) . '</i>';
				echo '<p><strong><i class="fas fa-user" style="margin-right: 10px;"></i></strong>' . $comment['email'] . ' / ' . $comment['Sao'] . '<i class="fas fa-star"></i>'. '</p>';
