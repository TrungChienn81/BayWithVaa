...
<?php
// Kiểm tra nếu có tham số location trên URL
if(isset($_GET['location'])) {
    // Lấy giá trị của tham số location
    $location = $_GET['location'];
    
    // Truy vấn các tour có DiaDiemToi là địa điểm được chọn
    $query = "SELECT * FROM tours WHERE DiaDiemToi = '$location'";
    ...
    echo '<i style="font-size: 30px;">Danh sách các tour đến ' . $location . '</i>';
	echo '<div style="display: flex; flex-warp: warp; justify-content: space-evenly">';
    // Hiển thị các tour
    ...
    echo '<p>Vui lòng chọn một địa điểm để xem các tour.</p>';
}
echo '</div>';
